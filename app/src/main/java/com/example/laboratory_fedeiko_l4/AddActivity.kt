...
import com.example.laboratory_fedeiko_l4.databinding.ActivityAddBinding
...
class AddActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(user == null) binding.btnAddOrrUpdateUser.text = "Add User"
        else {
            binding.btnAddOrrUpdateUser.text = "Update"
            binding.edFirstName.setText(user?.firstName.toString())
            binding.edLastName.setText(user?.lastName.toString())
        }

        binding.btnAddOrrUpdateUser.setOnClickListener {
            addUser()
        }
    }
